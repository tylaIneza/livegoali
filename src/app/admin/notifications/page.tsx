    ...
    <div className="space-y-6">
      ...
      {notifications.length === 0 ? (
        <div className="rounded-2xl border border-white/8 bg-[#121821] py-16 text-center">
          ...
          <p className="text-white/70 text-sm">No notifications sent yet.</p>
        </div>
      ) : (
              ...
              <div key={n.id} className="flex items-start gap-4 px-4 py-3 hover:bg-white/2 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs text-white/60 shrink-0">{formatTimeAgo(n.createdAt)}</span>
                  </div>
                  <p className="text-xs text-white/75 mt-0.5">{n.message}</p>
                  <p className="text-[10px] text-white/60 mt-1">→ {n.user.name || n.user.email}</p>
                </div>
              </div>
            ))}
          </div>
